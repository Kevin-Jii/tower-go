...
func RegisterMemberRoutes(v1 *gin.RouterGroup, c *Controllers) {
	...
	members.Use(middleware.AuthMiddleware(), middleware.StoreBusinessGuard())
	{
		...
		members.GET("/stats", middleware.Permission("store:member:list"), c.Member.MemberStats)
		...
		members.GET("/:id/consumptions/export", middleware.Permission("store:member:list"), c.Member.ExportMemberConsumptions)
