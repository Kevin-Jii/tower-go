...
func BatchInsert(db *gorm.DB, records interface{}, batchSize ...int) error {
	...
	if err != nil {
		LogError("批量插入失败", zap.Error(err))
		return err
	}

	LogInfo("批量插入成功", zap.Int("batch_size", size))
	return nil
}
...
func BatchUpdate(db *gorm.DB, model interface{}, ids []uint, updates map[string]interface{}) error {
	...
	if err != nil {
		LogError("批量更新失败", zap.Error(err), zap.Int("count", len(ids)))
		return err
	}

	LogInfo("批量更新成功", zap.Int("count", len(ids)))
	return nil
}
...
func BatchDelete(db *gorm.DB, model interface{}, ids []uint) error {
	...
	if err != nil {
		LogError("批量删除失败", zap.Error(err), zap.Int("count", len(ids)))
		return err
	}

	LogInfo("批量删除成功", zap.Int("count", len(ids)))
	return nil
}
...
func BatchHardDelete(db *gorm.DB, model interface{}, ids []uint) error {
	...
	if err != nil {
		LogError("批量硬删除失败", zap.Error(err), zap.Int("count", len(ids)))
		return err
	}

	LogInfo("批量硬删除成功", zap.Int("count", len(ids)))
	return nil
}
...
func BatchOperation(db *gorm.DB, operation func(tx *gorm.DB) error) error {
	return db.Transaction(func(tx *gorm.DB) error {
		if err := operation(tx); err != nil {
			LogError("批量操作失败，事务已回滚", zap.Error(err))
			return err
		}
		LogInfo("批量操作成功，事务已提交")
		return nil
	})
}
...
func ChunkProcess(db *gorm.DB, model interface{}, chunkSize int, processor func(records interface{}) error) error {
	...
	for {
		...
		if err != nil {
			LogError("分块查询失败", zap.Error(err), zap.Int("offset", offset))
			return err
		}
		...
		if err := processor(records); err != nil {
			LogError("分块处理失败", zap.Error(err), zap.Int("offset", offset))
			return err
		}
		...
		offset += chunkSize
		LogDebug("分块处理进度", zap.Int("processed", offset))
	}

	LogInfo("分块处理完成", zap.Int("total_processed", offset))
	return nil
}
...
func BulkUpsert(db *gorm.DB, records interface{}, conflictColumns []string, updateColumns []string) error {
	...
	if err != nil {
		LogError("批量 Upsert 失败", zap.Error(err))
		return err
	}

	LogInfo("批量 Upsert 成功")
	return nil
}
