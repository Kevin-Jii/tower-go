...
export const useUserStore = defineStore("user", () => {
  function hasInventoryLossMenu(list: Menu[] | undefined): boolean {
    ...
    for (const m of list) {
      if (
        m.path === "/store/inventory-loss" ||
        m.component === "store/inventory-loss/index"
      )
        return true;
  ...
  function attachInventoryLossMenu(list: Menu[]): Menu[] {
    ...
    const storeNode = cloned.find(
      (m) => m.type === 1 && (m.path === "/store" || m.name === "store"),
    );
    if (storeNode) {
      storeNode.children = storeNode.children ?? [];
      storeNode.children.push({ parent_id: storeNode.id } as Menu);
      return cloned;
    }
    return cloned;
  }
  ...
  function setMenus(list: Menu[]): void {
    menus.value = attachInventoryLossMenu(list ?? []);
  }
