...
type ProductUnitSpec struct {
	...
	IsSaleable   bool                        `json:"is_saleable" gorm:"not null;default:true;comment:是否为可售卖规格"`
	IsEnabled    bool                        `json:"is_enabled" gorm:"not null;default:true;comment:是否启用"`
	Consumables  []ProductUnitSpecConsumable `json:"consumables,omitempty" gorm:"foreignKey:UnitSpecID"`
...
type UpdateProductUnitSpecReq struct {
	...
	IsSaleable   *bool    `json:"is_saleable,omitempty"`
...
type CreateProductUnitSpecUnitItem struct {
	...
	IsSaleable   *bool                              `json:"is_saleable"`
	IsEnabled    *bool                              `json:"is_enabled"`
	Consumables  []ProductUnitSpecConsumableItemReq `json:"consumables" binding:"omitempty,dive"`
}

// ProductUnitSpecConsumable 商品规格在单个门店下的消耗品用量配置。
type ProductUnitSpecConsumable struct {
	ID                  uint                           `json:"id" gorm:"primaryKey;autoIncrement"`
	UnitSpecID          uint                           `json:"unit_spec_id" gorm:"not null;index;uniqueIndex:uk_unit_spec_store_consumable,priority:1;comment:商品规格ID"`
	StoreID             uint                           `json:"store_id" gorm:"not null;index;uniqueIndex:uk_unit_spec_store_consumable,priority:2;comment:门店ID"`
	ConsumableProductID uint                           `json:"consumable_product_id" gorm:"not null;index;uniqueIndex:uk_unit_spec_store_consumable,priority:3;comment:消耗品档案ID"`
	Quantity            float64                        `json:"quantity" gorm:"type:decimal(12,4);not null;default:1;comment:每售卖一件规格消耗数量"`
	ConsumableProduct   *StoreAccountConsumableProduct `json:"consumable_product,omitempty" gorm:"foreignKey:ConsumableProductID"`
	CreatedAt           time.Time                      `json:"created_at"`
	UpdatedAt           time.Time                      `json:"updated_at"`
}

func (ProductUnitSpecConsumable) TableName() string {
	return "product_unit_spec_consumables"
}

type ProductUnitSpecConsumableItemReq struct {
	ConsumableProductID uint    `json:"consumable_product_id" binding:"required"`
