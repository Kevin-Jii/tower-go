...
func InitDingTalkMenuSeeds(db *gorm.DB) error {
	...
	if count > 0 {
		LogInfo("钉钉管理菜单已存在，跳过初始化", zap.Int64("count", count))
		return nil
	}
	...
	if err := db.Create(&dingTalkMenus).Error; err != nil {
		LogError("创建钉钉管理菜单失败", zap.Error(err))
		return err
	}

	LogInfo("✅ 钉钉管理菜单创建成功", zap.Int("count", len(dingTalkMenus)))

	// 为管理员角色分配权限
	if err := assignDingTalkMenusToAdmin(db); err != nil {
		LogError("分配钉钉菜单权限失败", zap.Error(err))
		return err
	}
...
func assignDingTalkMenusToAdmin(db *gorm.DB) error {
	...
	LogInfo("✅ admin 角色钉钉权限分配成功", zap.Uint("role_id", adminRole.ID))
	...
	if superAdminRole.ID > 0 {
		...
		LogInfo("✅ 超级管理员钉钉权限分配成功", zap.Uint("role_id", superAdminRole.ID))
	}
