...
func CacheGet(key string, dest interface{}) error {
	...
	if err := json.Unmarshal([]byte(val), dest); err != nil {
		LogError("缓存反序列化失败", zap.String("key", key), zap.Error(err))
		return err
	}

	LogDebug("缓存命中", zap.String("key", key))
	return nil
}
...
func CacheSet(key string, value interface{}, ttl time.Duration) error {
	...
	if err != nil {
		LogError("缓存序列化失败", zap.String("key", key), zap.Error(err))
		return err
	}

	if err := RedisClient.Set(ctx, key, jsonData, ttl).Err(); err != nil {
		LogError("缓存设置失败", zap.String("key", key), zap.Error(err))
		return err
	}

	LogDebug("缓存设置成功", zap.String("key", key), zap.Duration("ttl", ttl))
	return nil
}
...
func CacheDelete(keys ...string) error {
	...
	if err := RedisClient.Del(ctx, keys...).Err(); err != nil {
		LogError("缓存删除失败", zap.Strings("keys", keys), zap.Error(err))
		return err
	}

	LogDebug("缓存删除成功", zap.Strings("keys", keys))
	return nil
}
...
func CacheDeletePattern(pattern string) error {
	...
	if err := iter.Err(); err != nil {
		LogError("缓存模式扫描失败", zap.String("pattern", pattern), zap.Error(err))
		return err
	}

	if len(keys) > 0 {
		if err := RedisClient.Del(ctx, keys...).Err(); err != nil {
			LogError("缓存批量删除失败", zap.String("pattern", pattern), zap.Error(err))
			return err
		}
		LogInfo("缓存批量删除成功", zap.String("pattern", pattern), zap.Int("count", len(keys)))
	}
...
func CacheGetOrSet(key string, dest interface{}, ttl time.Duration, fetchFunc func() (interface{}, error)) error {
	...
	if err := CacheSet(key, data, ttl); err != nil {
		LogWarn("缓存设置失败，但数据已获取", zap.String("key", key), zap.Error(err))
	}
